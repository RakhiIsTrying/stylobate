...
async def _impl_search_news(
    ticker: str, limit: int = 10, market: str = "US"
) -> NewsResult:
    items = await _fetch_news_for_ticker(ticker=ticker, limit=limit, market=market)
...
search_news_tool = Tool(
    ...
    description=(
        ...
        "looks up the right symbol; 'IN' for Indian tickers; 'US' (default) otherwise."
    ),
    input_schema={
        "type": "object",
        "properties": {
